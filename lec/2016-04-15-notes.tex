...
%% \section*{Gauss-Newton, Newton, and IRLS}

%% At the end of last lecture, we discussed the Gauss-Newton and
%% Levenberg-Marquardt iterations in the context of the trust region
%% idea.  We also stated briefly that the Gauss-Newton iteration is
%% not the same as Newton iteration, and the difference can be
%% significant when there is a large residual.  I did not, however,
%% provide any sort of convergence bounds or analysis.
...
\begin{tikzpicture}
  \begin{axis}[width=\textwidth,height=5cm,xlabel={$\gamma$},ylabel={$v(0.5)$},
      scaled y ticks=base 10:2]
    \addplot table {2016-04-15-codes/vcenterm.txt};
  \end{axis}
\end{tikzpicture}
...
\lstinputlisting{2016-04-15-codes/blowupc.m}

\subsection*{Continuation in $\mu$}

\lstinputlisting{2016-04-15-codes/blowupmid.m}
